...
This skill integrates with the agent registry (`.Codex/pilot/agent-registry.json`) when `--orchestrate` flag is used.
...
- Loads agent rules from `.Codex/pilot/rules/*.yaml`
...
const registry = JSON.parse(readFile('.Codex/pilot/agent-registry.json'));
