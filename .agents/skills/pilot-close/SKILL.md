...
Load `.Codex/pilot/policy.yaml` and check `autonomy` section.

If `autonomy.mode` is `"full"` AND `autonomy.auto_chain_on_close` is `true`:

### 9.1: Update autonomous state
Update `.Codex/pilot/state/autonomous.json`:
...
```bash
node .Codex/pilot/hooks/cli/release-task.js 2>/dev/null
```
...
```bash
node .Codex/pilot/hooks/cli/list-claimed.js 2>/dev/null
```

### 9.4: Chain to next task
If an unclaimed ready task exists:
1. Claim it: `bd update {id} --status in_progress && node .Codex/pilot/hooks/cli/claim-task.js {id}`
2. Create implementation plan (inline, same as /pilot-plan)
3. Auto-approve the plan (write approval file to `.Codex/pilot/state/approved-plans/`)
