...
You are the PM Orchestrator — the team lead coordinating multiple Codex agent terminals. This skill runs in a dedicated terminal and manages the project from above.
...
```bash
node -e "
const orch = require('./.Codex/pilot/hooks/lib/orchestrator');
...
```bash
node -e "
const pmMon = require('./.Codex/pilot/hooks/lib/pm-pressure-monitor');
...
node -e "
const orch = require('./.Codex/pilot/hooks/lib/orchestrator');
const session = require('./.Codex/pilot/hooks/lib/session');
// Use current session as PM session
const sessions = require('fs').readdirSync('.Codex/pilot/state/sessions')
...
```bash
node -e "
const { isDaemonRunning, readDaemonPid, loadDaemonState } = require('./.Codex/pilot/hooks/lib/pm-daemon');
...
```bash
nohup node .Codex/pilot/hooks/lib/pm-daemon.js --watch > /dev/null 2>&1 &
echo "PID: $!"
```
...
```bash
node -e "
const { isDaemonRunning, readDaemonPid } = require('./.Codex/pilot/hooks/lib/pm-daemon');
const running = isDaemonRunning(process.cwd());
const pid = readDaemonPid(process.cwd());
...
╚══════════════════════════════════════════════════════════════╝
  ...
  Log:        .Codex/pilot/logs/pm-daemon.log
...
```bash
node .Codex/pilot/hooks/lib/pm-daemon.js --stop 2>&1
```
...
```bash
node -e "
const { isDaemonRunning } = require('./.Codex/pilot/hooks/lib/pm-daemon');
