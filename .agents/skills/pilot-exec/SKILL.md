...
Before executing, load `.Codex/pilot/policy.yaml` and check the `autonomy` section.
...
```bash
node -e "
const { loadConfig, runPipeline } = require('./.Codex/pilot/hooks/lib/test-gen-integration');
...
- The pipeline analyzes staged changes, detects test framework, generates tests via Codex -p
