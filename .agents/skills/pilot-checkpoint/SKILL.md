...
### 1.1: Get session ID
```bash
SESSION_FILE=$(ls -t .Codex/pilot/state/sessions/S-*.json 2>/dev/null | grep -v pressure | head -1)
...
```bash
node -e "
  const p = require('./.Codex/pilot/hooks/lib/pressure');
...
```bash
node -e "
  const cp = require('./.Codex/pilot/hooks/lib/checkpoint');
