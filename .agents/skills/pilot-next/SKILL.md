...
```bash
node .Codex/pilot/hooks/cli/next-task.js 2>/dev/null
```
...
**Check autonomy mode first**: Load `.Codex/pilot/policy.yaml` and check `autonomy` section.
...
This prevents other Codex sessions from working on the same task.

```bash
bd update {id} --status in_progress && node .Codex/pilot/hooks/cli/claim-task.js {id}
```
...
- Auto-approve the plan immediately:
  1. Write approval file to `.Codex/pilot/state/approved-plans/{taskId}.json` with `auto_approved: true`
  2. Update `.Codex/pilot/state/autonomous.json` with `running: true`, `currentTask: {taskId}`
  3. Display:
     ```
...
To release a task: `node .Codex/pilot/hooks/cli/release-task.js`
