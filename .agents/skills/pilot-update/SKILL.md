...
```bash
cat .Codex/pilot/VERSION 2>/dev/null || echo "unknown"
```

Also check the VERSION.lock for install metadata:
```bash
cat .Codex/pilot/VERSION.lock 2>/dev/null
```
...
```bash
node -e "try{const v=JSON.parse(require('fs').readFileSync('.Codex/pilot/VERSION.lock','utf8'));console.log(v.install_type||'local')}catch{console.log('local')}"
```
