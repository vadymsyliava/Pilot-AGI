...
```bash
SESSION_FILE=$(ls -t .Codex/pilot/state/sessions/S-*.json 2>/dev/null | head -1)
...
```bash
node -e "
const m = require('./.Codex/pilot/hooks/lib/messaging');
...
```bash
node -e "
const ac = require('./.Codex/pilot/hooks/lib/agent-context');
...
```bash
node -e "
const memory = require('./.Codex/pilot/hooks/lib/memory');
